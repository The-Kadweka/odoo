...
class MoneyController(http.Controller):
    ...
    def view_goal_details(self, **kw):
        ...
        if not goal_id:
            ...
            return response
        token = request.env['jwt_provider.access_token'].sudo().search(
            [('is_expired', '!=', True), ('token', '=', access_token)])
        if token:
            goal_id_data = request.env['personal.goals'].sudo().search(
                [('id', '=', data['goal_id']), ('partner_id.email', '=', token.partner_id.email)])
            if goal_id_data:
    ...
    def remove_expense(self, **kw):
        ...
        token = request.env['jwt_provider.access_token'].sudo().search(
            [('is_expired', '!=', True), ('token', '=', access_token)])
        if token:
            expense_id = request.env['account.spend'].sudo().search(
                [('id', '=', expense_id), ('partner_id.id', '=', token.partner_id.id)])
            if expense_id.spent_on == "INCOME":
                expense_id.account_id.sudo().write(
                    {"balance": expense_id.account_id.balance-expense_id.amt})
            ...
            if expense_id.spent_on == "SAVINGS":
                expense_id.account_id.sudo().write(
                    {"balance": expense_id.account_id.balance-expense_id.amt})
                expense_id.goal_id.sudo().write(
                    {"current_saving": expense_id.goal_id.current_saving-expense_id.amt})
                expense_id.sudo().unlink()
                response = {
                    'code': 200,
                    'message': 'You Have deleted an Expense'
    ...
    def remove_goal(self, **kw):
        ...
        token = request.env['jwt_provider.access_token'].sudo().search(
            [('is_expired', '!=', True), ('token', '=', access_token)])
        if token:
            goal_id = request.env['personal.goals'].sudo().search(
                [('id', '=', goal_id), ('partner_id.id', '=', token.partner_id.id)])
    ...
    def view_goals(self, **kw):
        ...
        token = request.env['jwt_provider.access_token'].sudo().search(
            [('is_expired', '!=', True), ('token', '=', access_token)])
        account_id = request.env['personal.goals'].sudo().search(
            [('partner_id.email', '=', token.partner_id.email)])
        if token:
    ...
    def view_transactions(self, **kw):
        ...
        token = request.env['jwt_provider.access_token'].sudo().search(
            [('is_expired', '!=', True), ('token', '=', access_token)])
        account_id = request.env['users.account'].sudo().search(
            [('partner_id.email', '=', token.partner_id.email)])
        if token:
            ...
            category.append(
                {"name": "House Holds", "amount": round(households, 2)})
            category.append({"name": "Vacation", "amount": round(vacation, 2)})
            category.append(
                {"name": "Transport", "amount": round(transport, 2)})
            category.append(
                {"name": "Subscription", "amount": round(subscription, 2)})
            category.append({"name": "Savings", "amount": round(savings, 2)})
            category.append({"name": "Renting", "amount": round(rent, 2)})
            category.append({"name": "Food/Groceries",
                            "amount": round(shopping, 2)})
            category.append(
                {"name": "Entertainment", "amount": round(entertainment, 2)})
            category.append({"name": "Income", "amount": round(income, 2)})
            category.append(
                {"name": "Education", "amount": round(education, 2)})
            category.append(
                {"name": "Electronics", "amount": round(electronics, 2)})
    ...
    def view_filtered_data(self, **kw):
        ...
        if token:
            if data['period'] == "Today":
                expenses = account_id.expenditure.sudo().search(
                    [("date", "=", date), ('account_id.partner_id.email', '=', token.partner_id.email)])
                for rec in expenses:
                    if rec.spent_on != "INCOME":
                        spending += rec.amt
                    if rec.spent_on == "BILL":
                    ...
                    if rec.spent_on == "MEDICATIONS":
                        meds += rec.amt
                category.append({"name": "Others", "amount": round(others, 2), "per": round(
                    others, 2) if others == 0.00 else round(others/spending*100, 1)})
                category.append({"name": "Bill", "amount": round(bill, 2), "per": round(
                    0.00) if bill == 0.00 else round(bill/spending*100, 1)})
                category.append({"name": "House Holds", "amount": round(households, 2), "per": round(
                    0.00) if households == 0.00 else round(households/spending*100, 1)})
                category.append({"name": "Vacation", "amount": round(vacation, 2), "per": round(
                    0.00) if households == 0.00 else round(vacation/spending*100, 1)})
                category.append({"name": "Transport", "amount": round(transport, 2), "per": round(
                    0.00) if transport == 0.00 else round(transport/spending*100, 1)})
                category.append({"name": "Subscription", "amount": round(subscription, 2), "per": round(
                    0.00) if subscription == 0.00 else round(subscription/spending*100, 1)})
                category.append({"name": "Renting", "amount": round(rent, 2), "per": round(
                    0.00) if rent == 0.00 else round(rent/spending*100, 1)})
                category.append({"name": "Food/Groceries", "amount": round(shopping, 2),
                                "per": round(0.00) if shopping == 0.00 else round(shopping/spending*100, 1)})
                category.append({"name": "Savings", "amount": round(saves, 2), "per": round(
                    0.00) if saves == 0.00 else round(saves/spending*100, 1)})
                category.append({"name": "Entertainment", "amount": round(entertainment, 2), "per": round(
                    0.00) if entertainment == 0.00 else round(entertainment/spending*100, 1)})
                category.append({"name": "Education", "amount": round(education, 2), "per": round(
                    0.00) if education == 0.00 else round(education/spending*100, 1)})
                category.append({"name": "Electronics", "amount": round(electronics, 3), "per": round(
                    0.00) if electronics == 0.00 else round(electronics/spending*100, 1)})
                category.append({"name": "Charity", "amount": round(charity, 2), "per": round(
                    0.00) if charity == 0.00 else round(charity/spending*100, 1)})
                category.append({"name": "Clothing", "amount": round(clothing, 2), "per": round(
                    0.00) if clothing == 0.00 else round(clothing/spending*100, 1)})
                category.append({"name": "Medication", "amount": round(meds, 2), "per": round(
                    0.00) if meds == 0.00 else round(meds/spending*100, 1)})
                category.append({"name": "Airtime", "amount": round(credit, 2), "per": round(
                    0.00) if credit == 0.00 else round(credit/spending*100, 1)})
                return {
                    "code": 200,
                    "status": "successfully",
                    "expense": spending,
            ...
            if data['period'] == "thisMonth":
                expenses = account_id.expenditure.sudo().search(
                    [('account_id.partner_id.email', '=', token.partner_id.email)])
                for rec in expenses:
                    ...
                    if rec.spent_on == "MEDICATIONS" and rec.date.month == datetime.strptime(str(date), '%m/%d/%Y').month:
                        meds += rec.amt
                category.append({"name": "Others", "amount": round(others, 2), "per": round(
                    0.00) if others == 0.00 else round(others/spending*100, 1)})
                category.append({"name": "Bill", "amount": round(bill, 2), "per": round(
                    0.00) if bill == 0.00 else round(bill/spending*100, 1)})
                category.append({"name": "House Holds", "amount": round(households, 2), "per": round(
                    0.00) if households == 0.00 else round(households/spending*100, 1)})
            ...
            if data['period'] == "past6Month":
                date_6_ago = (datetime.strptime(
                    str(date), '%m/%d/%Y') + relativedelta(months=-6))
                expenses = account_id.expenditure.sudo().search([("date", ">=", date_6_ago), ("date", "<=", datetime.strptime(
                    str(date), '%m/%d/%Y')), ('account_id.partner_id.email', '=', token.partner_id.email)])
                for rec in expenses:
                    if rec.spent_on == "BILL":
                        bill += rec.amt
                ...
                if spent_on == "INCOME":
                    ...
                    if expense:
                        account_id.sudo().write(
                            {"balance": account_id.balance+expense.amt})
                        return {
                            "code": 200,
                ...
                if spent_on == "SAVINGS":
                    if float(amount) > account_id.balance:
                        ...
                        if goal_id:
                            ...
                            if expense_id:
                                goal_id.sudo().write(
                                    {"current_saving": goal_id.current_saving+expense_id.amt})
                                account_id.sudo().write(
                                    {"balance": account_id.balance-expense_id.amt})
                                return {
                                    "code": 200,
                                    "status": "success",
                                    "type": data["spent_on"],
                    ...
                    if float(amount) > account_id.balance:
                        ...
                        if expense:
                            account_id.sudo().write(
                                {"balance": account_id.balance-float(amount)})
                            return {
                                "code": 200,
                                "record": account_id.balance,
                                "type": data["spent_on"],
